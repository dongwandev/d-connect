...
> 가입 이메일 인증(#86)·비밀번호 재설정(#88) 링크를 발송하기 위한 SMTP 설정 가이드.
...
4. 비밀번호 재설정: 로그인 → 계정 찾기 → "재설정 링크 받기" → `[D-Connect] 비밀번호 재설정 안내` 메일의
   버튼 클릭 → `/reset-password`에서 새 비밀번호 설정 (링크 1시간 유효, 일회용)
